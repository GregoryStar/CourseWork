...
RobotViewer::RobotViewer()
  ...
  gui->addButton("Change Color", std::bind(&RobotViewer::OnColorChangeBtnPressed, this));
...
void RobotViewer::OnColorChangeBtnPressed(){
  int currentColor = robot_land_->get_robot_color(0);
  robot_land_->set_robot_color(0, 1000000 + currentColor);
}
...
void RobotViewer::DrawRobot(NVGcontext *ctx, int id, double xpos, double ypos,
  ...
  int robotColors = robot_land_->get_robot_color(id);
  nvgFillColor(ctx, nvgRGBA((robotColors >> 24) & 0xFF, (robotColors >> 16) & 0xFF, (robotColors >> 8) & 0xFF, (robotColors >> 0) & 0xFF));
  nvgFill(ctx);
