...
class Robot : public ArenaMobileEntity {
 public:
  ...
  /**
   * @brief Returns the heading_angle_ of the Robot from the motion handler.
   */
  ...
  virtual void TimestepUpdate(uint dt) {
    ...
    if (isColliding == 1) {
      ...
      if (sensor_touch_.point_of_contact().y <= 0) {
        set_pos(Position(get_pos().x, 0 + get_radius() + 2));
      }
      if (sensor_touch_.point_of_contact().y >= 768) {
        set_pos(Position(get_pos().x, 768 - get_radius() - 2));
      }
      if (sensor_touch_.point_of_contact().x <= 0) {
        set_pos(Position(0 + get_radius() + 2, get_pos().y));
      }
      if (sensor_touch_.point_of_contact().x >= 1024) {
        set_pos(Position(1024 - get_radius() - 2, get_pos().y));
      }
    ...
    if (isDistress == 0 && !frozen_ && (proxDist <= 100 && proxDist >= 1)
      && !isSuperBot_) {
        std::cout << " proxDist " << proxDist;
        motion_handler_.set_heading_angle(motion_handler_.get_heading_angle()
          + 30);
        motion_handler_.set_speed(motion_handler_.max_speed()
          * (proxDist/(100*3)));
    } else {
