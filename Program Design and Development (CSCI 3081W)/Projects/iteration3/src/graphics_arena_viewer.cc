...
void GraphicsArenaViewer::DrawPlayer(NVGcontext *ctx,
  ...
  double battery_level = player->battery_level();
  ...
  // Creating a new text label in the top of the player if the game is over.
  nvgText(ctx, 0.0, 0.0, s.c_str(), NULL);
  if (arena_->gameLost()) {
    std::string win = "You Lose!";
      nvgText(ctx, 0.0, -30, win.c_str(), NULL);
  } else if (arena_->gameWon()) {
    std::string lose = "You Win!";
...
/**
 * @brief Basically identical to DrawObstacle but draws an object of type
 * Robot instead.
 */
void DrawRobot(NVGcontext *ctx, const Robot* const robot) {
  nvgBeginPath(ctx);
  nvgCircle(ctx, robot->get_pos().x, robot->get_pos().y,
    robot->get_radius());

  // nvgCircle(ctx, 150, 150,
  //  500);
  ...
  nvgFillColor(ctx, nvgRGBA(0, 0, 0, 255));
  nvgText(ctx, robot->get_pos().x, robot->get_pos().y,
