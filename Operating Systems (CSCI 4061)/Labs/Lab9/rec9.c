...
int main(int argc, char** argv) {
	...
	bank1->balance = 2500;
	bank1->lock = malloc(sizeof(pthread_mutex_t));
	
	if (pthread_mutex_init(bank1->lock, NULL) != 0) {
		printf("\n mutex init failed!\n");
		return 1;
	}
	...
	bank2->balance = 3000;
	bank2->lock = malloc(sizeof(pthread_mutex_t));
	if (pthread_mutex_init(bank2->lock, NULL) != 0) {
		printf("\n mutex init failed!\n");
		return 1;
	}
